...
fn parse(line: &str) -> Result<Pair, &str> {
    ...
    let dash_index = line
        .find("-")
        .ok_or("each line contains exactly one dash")?;
    ...
    Ok((
        Policy {
            ...
            letter: line[colon_index - 1..colon_index]
                .chars()
                .next()
                .expect("letter is a valid 1-char str"),
        },
...
fn passes_toboggan_rules(pair: &Pair) -> bool {
    ...
    let first = password
        .chars()
        .nth((policy.min - 1).into())
        .expect("First character index is valid");
    let second = password
        .chars()
        .nth((policy.max - 1).into())
        .expect("Second character index is valid");
